...
# Конструктор обещает, что ответы никуда не уходят, поэтому на документе, загруженном
# по его адресу, скрипт Метрики не запускается (решение владельца 2026-09-11, см.
# web/components/Metrika.tsx). Проверяем, что исключение доехало до собранной
# страницы: снимут его или адрес в нём разойдётся с маршрутом — гейт покраснеет, а не
# конструктор тихо окажется под записью.
grep -qF "location.pathname.indexOf('/tehzadanie') !== 0" "$OUT/tehzadanie/index.html" || {
  echo "::error::на /tehzadanie/ нет исключения для скрипта Метрики — ответы конструктора попадут в запись"
  exit 1
}
...
# Каждый URL из srcset обязан лежать файлом в выхлопе.
#
# Копии кадров собираются на сборке (web/scripts/gen-shots.mjs) и в репозиторий не
# коммитятся, а манифест `content/shots-generated.ts` — наоборот, коммитится ради
# `pnpm typecheck`. Значит, возможен разъезд: манифест обещает копию, которой не
# сгенерили. Браузер на такой srcset не ругается — он молча возьмёт `src`, и мы бы
# считали, что адаптивные кадры работают, когда они не работают. Проверяем файлами.
#
# Атрибут ищем в обоих регистрах: React 19 печатает его как `srcSet=`, а не
# `srcset=`. Для браузера это одно и то же (имена атрибутов регистронезависимы), но
# шаблон на нижний регистр находил ноль совпадений при полностью рабочей разметке —
# то есть проверка падала бы всегда. Классы вида [sS] здесь безопасны: ломались они
# только на кириллице (грабля 12), а тут чистый ASCII.
#
# Коды grep различаем явно, как и везде в этом файле: 0 — нашли, 1 — ни одного
# srcset (тоже ошибка: кадры обязаны быть адаптивными), >1 — сам grep не отработал.
set +e
srcset_attrs=$(grep -roh '[sS][rR][cC][sS][eE][tT]="[^"]*"' "$OUT" --include='*.html' 2>&1)
rc=$?
set -e
case "$rc" in
  0) : ;;
  1) echo "::error::в выхлопе нет ни одного srcset — кадры отдаются без адаптивных копий"; exit 1 ;;
  *) echo "::error::проверка srcset не отработала (grep rc=$rc): $srcset_attrs"; exit 1 ;;
esac

srcset_urls=$(printf '%s\n' "$srcset_attrs" \
  | sed 's/[sS][rR][cC][sS][eE][tT]="//; s/"$//' \
  | tr ',' '\n' \
  | sed 's/^[[:space:]]*//; s/[[:space:]][0-9]*w$//' \
  | sort -u)
...
while IFS= read -r url; do
  [ -n "$url" ] || continue
  checked=$((checked + 1))
  test -s "$OUT$url" || { echo "::error::srcset ссылается на файл, которого нет в выхлопе: $url"; missing=1; }
done <<EOF
$srcset_urls
EOF
test "$missing" -eq 0 || exit 1
echo "srcset цел: $checked кандидатов, все лежат файлами"
